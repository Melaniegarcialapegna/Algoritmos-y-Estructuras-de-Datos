...
const (
	TAM_INICIAL                   = 6
	redimensionarAumento          = 2
	redimensionarDisminucion      = 2
	multiplicadorReducirCapacidad = 4
)
...
func (pila *pilaDinamica[T]) Apilar(valor T) {
	if pila.cantidad == len(pila.datos) {
		pila.redimensionar(len(pila.datos) * redimensionarAumento)
	}
...
func (pila *pilaDinamica[T]) Desapilar() T {
	...
	pila.cantidad--
	if (!pila.EstaVacia()) && ((pila.cantidad * multiplicadorReducirCapacidad) <= len(pila.datos)) {
		pila.redimensionar(len(pila.datos) / redimensionarDisminucion)
	}
	return valor
}
